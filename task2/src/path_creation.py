...
query_create_paths_self_loop = """
    MATCH len=(j:Junction)-[:CONSISTS_OF]->(n:Node{time:$time})-[:SEGMENT{is_junction:$is_junction}]->(o:Node)-[:SEGMENT*{is_junction:$is_junction}]->(m:Node)<-[:CONSISTS_OF]-(j), 
    (n)<-[:CONSISTS_OF]-(l:Loop)-[:CONSISTS_OF]->(m), 
    (o)<-[:CONSISTS_OF]-(l) 
...
for time in range(50,10000,50):
    ...
    run_query(query_create_paths_standard, params)
    run_query(query_create_paths_self_loop, params)
    run_query(query_create_paths_length_one, params)
